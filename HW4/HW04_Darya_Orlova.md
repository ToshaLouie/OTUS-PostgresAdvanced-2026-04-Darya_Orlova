...
Развернула 7 VM в Yandex Cloud  в одной subnet.
Шесть машин не имееют public ip , а машинка для haproxy имеет public ip.
В качестве ОС выбрана - Ubuntu 24.04
Диск 10 гб, 2 cpu  и 2 gb Ram.
...
patroni1-vm: 10.129.0.31
patroni2-vm: 10.129.0.30
patroni3-vm: 10.129.0.37

haproxy:

haproxy - public ip : 158.160.18.170 private ip: 10.129.0.33

![vms.png](vms.png)
Рисунок 1 - Созданные виртуальные машины.

Выполнила общие настройки для всех машин:

1) Внесла правки в файле /etc/hosts где прописала все машинки необходимы нам для настройки кластера пантрони.

    10.129.0.18 etcd1-vm
    10.129.0.19 etcd-vm2
    10.129.0.21 etcd3-vm
...
2) В nsg сети открыла порты. Так как яндекс при создании портов не захотел применять настройку доступа только для сети
![nsg.png](nsg.png)
Рисунок 2 - открытие портов.

3) Обновила пакеты и установила одинаковую таймзону

    sudo apt update && sudo apt upgrade -y
    sudo apt install -y curl wget vim net-tools chrony mc vim
...
### Ход действий:
На машинках etcd1-vm - etcd3-vm произвела установку etcd.

    sudo apt install -y etcd-server etcd-client
    sudo systemctl stop etcd

После чего внесла на трех серверах правку в файл /etc/default/etcd
Для etcd-1 конфиг выглядит следующим образом:

    odd@etcd1-vm:~$ cat /etc/default/etcd 
...
Для etcd-2 конфиг выглядит следующим образом:
    ...
    odd@etcd-vm2:~$ cat /etc/default/etcd 
    ETCD_NAME="etcd-vm2"
    ETCD_DATA_DIR="/var/lib/etcd"
...
Для etcd-3 конфиг выглядит следующим образом:
    ...
    odd@etcd3-vm:~$ cat /etc/default/etcd 
    ETCD_NAME="etcd3-vm"
    ETCD_DATA_DIR="/var/lib/etcd"
...
![2_etcd_check.png](2_etcd_check.png)
Рисунок 3 - Проверка коррекности работы etcd

На машинках patroni1-vm-patroni3-vm выполнила следующее:
1)  Установила postgre + patroni
...
3) И настроила на каждой патрони конфиг /etc/patroni/patroni.yml для патрони:

Для patroni1-vm конфиг выглядит следующим образом:

    odd@patroni1-vm:~$ sudo cat /etc/patroni/patroni.yml
    scope: postgres-ha
    namespace: /service/
    name: patroni1-vm
...
Для patroni2-vm конфиг выглядит следующим образом:

    odd@patroni2-vm:~$ sudo cat /etc/patroni/patroni.yml
    scope: postgres-ha
    namespace: /service/
    name: patroni2-vm
...
Для patroni3-vm конфиг выглядит следующим образом:

    odd@patroni3-vm:~$ sudo cat /etc/patroni/patroni.yml
    scope: postgres-ha
    namespace: /service/
    name: patroni3-vm
...
4) На всех машинках патрони добавила автозапуск службы с патрони
...
6) Запустила сервис и проверила работопособность кластера
...
![3_patroni_setup_check.png](3_patroni_setup_check.png)
Рисунок 4 - Проверка работоспособности кластера патрони
...
1) Установила на машинку с именем haproxy компоненты для haproxy и настроила конфиг

        sudo apt install -y haproxy

        odd@haproxy:~$ sudo cat /etc/haproxy/haproxy.cfg
...
2) Запустила и проверила haproxy:
...
![4_status_ha_proxy.png](4_status_ha_proxy.png)
Рисунок 5 - Проверка haproxy

## Задание 4
## Проверьте отказоустойчивость кластера, имитируя сбой на одном из узлов.
### Ход действий:

1) C машины haproxy подключилась к кластеру пантрони. Создала базу otus и таблицу test.
...
2)  После чего выключила первую патрони - которая выступала primary серером.
3)  Внесла INSERT строку.
...
4)  Включила обратно первый патрони и убедилась, что он стал репликой и данные корректно отображаются.
...
В ходе 4-ого задания был успешно смоделирован отказ primary-узла, после чего кластер автоматически выполнил failover, назначил новый primary и сохранил доступность базы данных без потери данных.
Задание 5 будет выполнено в рамках ДЗ 5
